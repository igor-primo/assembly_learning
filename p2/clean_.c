...
double *create_matrix(i64 lins, i64 cols){
    double *m = (double *) malloc(lins*cols*sizeof(double));
    i64 mult = lins*cols;
    for(i64 i=0;i<mult;i++)
        m[i] = 0.0;
    return m;
}
...
void show(double **m, i64 lins, i64 cols){
    for(i64 i=0;i<lins;i++){
...
int main(int argc, char **argv){
    ...
    while(tests--){
        ...
        fprintf(out, "M%d:\n", trick - (tests + 1));
        for(i64 i=0;i<cols2;i++){
            for(i64 j=0;j<lins1;j++)
