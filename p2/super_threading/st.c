...
void *read(void *id){
    ...
    while(tests--){
       ...
       // create matrix 1
       tasks[index].a = (double **) malloc(tasks[index].lins1 * sizeof(double *));
       for(i64 i=0;i<tasks[index].lins1;i++){
            tasks[index].a[i] = (double *) malloc(tasks[index].cols1 * sizeof(double));
            for(i64 j=0;j<tasks[index].cols1;j++)
                fscanf(in, "%lf", &(tasks[index].a[i][j]));
       }
       fscanf(in, "%"SCNu64"""%"SCNu64"", &(tasks[index].lins2), \
               &(tasks[index].cols2));
       printf("creating matrix 2\n");
       // create matrix 2
       tasks[index].b = (double **) malloc(tasks[index].lins2 * sizeof(double *));
       for(i64 i=0;i<tasks[index].lins2;i++){
            tasks[index].b[i] = (double *) malloc(tasks[index].cols2 * sizeof(double));
            for(i64 j=0;j<tasks[index].cols2;j++)
                fscanf(in, "%lf", &tasks[index].b[i][j]);
       }
       printf("creating matrix 3\n");
        // create matrix result
       tasks[index].c = (double **) malloc(tasks[index].lins1 * sizeof(double *));
       for(i64 i=0;i<tasks[index].lins1;i++){
            tasks[index].c[i] = (double *) malloc(tasks[index].cols2 * sizeof(double));
            for(i64 j=0;j<tasks[index].cols2;j++)
                tasks[index].c[i][j] = 0.0;
       }
