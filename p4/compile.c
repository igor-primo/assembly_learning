...
uint8_t which_operand(int8_t rx, int8_t ry, uint8_t *prefix){
    uint8_t operand = 0;
    int8_t source = 0, destination = 0;

    // mod | ry | rx
    operand |= 0xc0; // operand == 0b11000000
    source = ry > 7 ? ry % 8 : ry; // 000 <= source <= 111
    destination = rx > 7 ? rx % 8 : rx;
    *prefix = ry > 7 ? 0x49 : 0x48;
    source <<= 3;
    operand = source | destination;

    printf("which_operand = %d\n", operand);
    return operand;
}
...
void execute(){
    ...
    const uint64_t (*jit)() = (const uint64_t(*)())(memory);
    (*jit)();
